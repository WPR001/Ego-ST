...
#################
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            all_completions = unwrapped_model.generate(
                **prompt_inputs,
            ...
            num_generations = self.generation_config.num_return_sequences
            all_completions = all_completions.reshape(batch_size, num_generations, -1)
        ...
        # by wpr 04/03/2025
        # maybe we can use another way to normalize the rewards which is more reasonable.
        # if all rewards are negative, we should consider use a better normalization method.
