...
for idx, (json_file, video_folder) in enumerate(zip(input_json_files, video_folders)):
    ...
    for key, sample in data.items():
        if "options" not in sample["forward_mcq"] or not sample["forward_mcq"]["options"]:
            print(f'Skipping sample {key} from {json_file} because forward_mcq options are empty.')
        ...
        options_str = "\n".join(options)
        question = (video_prefix + sample["forward_mcq"]["question"] + "\n\nOptions:\n" + options_str +
...
output_path = './forward_mcq_output_action.json'
